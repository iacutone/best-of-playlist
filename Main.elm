module Main exposing (Model, Msg(..), Song, animateOpacity, audioSong, init, initialModel, main, onEnd, onTimeUpdate, songActions, songToPlay, subscriptions, targetCurrentTime, targetId, update, view, viewCircle, viewNextButton, viewPlayButton, viewPlaylist, viewPreviousButton, viewRadioInput, viewSongs, viewTitle)
...
import Svg.Attributes exposing (cx, cy, fill, r, stroke, strokeDasharray, strokeDashoffset, strokeWidth)
    ...
    case msg of
        Select playlist ->
            case playlist of
                ...
                "2019" ->
                    ( { model | songs = songs2019, activePlaylist = True, playing = False, playlist = playlist, style = animateOpacity model.currentBrowserTime }, Cmd.none )

                "2020" ->
                    ( { model | songs = songs2020, activePlaylist = True, playing = False, playlist = playlist, style = animateOpacity model.currentBrowserTime }, Cmd.none )

                "2021" ->
                    ( { model | songs = songs2021, activePlaylist = True, playing = False, playlist = playlist, style = animateOpacity model.currentBrowserTime }, Cmd.none )
                ...
                "2024" ->
                    ( { model | songs = songs2024, activePlaylist = True, playing = False, playlist = playlist, style = animateOpacity model.currentBrowserTime }, Cmd.none )

                "2025" ->
                    ( { model | songs = songs2025, activePlaylist = True, playing = False, playlist = playlist, style = animateOpacity model.currentBrowserTime }, Cmd.none )
        ...
        PlayPreviousSong ->
            if model.position == toFloat 1 then
                let
                    playlistLength =
                        toFloat (List.length model.songs)

                    song =
                        "song" ++ toString playlistLength
                in
                ...
                let
                    song =
                        "song" ++ toString (model.position - 1)
                in
...
viewPlaylist model =
    let
        opacity =
            Animation.animate model.currentBrowserTime model.style
    in
    div []
...
viewCircle model =
    let
        ...
        percentComplete =
            ((model.duration - model.currentTime) / model.duration) * 100

        pctInt =
            ((100 - percentComplete) / 100) * cInt
...
viewNextButton playing =
    button
        [ class "fa fa-forward btn-forward"
